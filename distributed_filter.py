            ...
            for line in mission_emails:
                ...
                if temp % 20 == 0:
                    ...
                    if c == 250:
                        ...
                        log.debug('{} - {}'.format(thread_id, line))
                if success % 100 == 0:
                    requests.get('http://127.0.0.1:5004/success_number/')
                if temp % 100 == 0:
                    requests.get('http://127.0.0.1:5004/filter_number/')
                temp += 1
            log.debug('send back email data')
            data = json.loads({
                'emails': result
            })
            requests.post('http://127.0.0.1:5004/result/', data=data)
...
def get_mission():
    response = requests.get('http://127.0.0.1:5004/filter/')
    data = response.json()
